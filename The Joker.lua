...
function Ronin:mySpells()
	self.Q = { range = 400, width = myHero:GetSpellData(_Q).width, delay = myHero:GetSpellData(_Q).delay, speed = myHero:GetSpellData(_Q).speed }
	self.W = { range = 500, width = myHero:GetSpellData(_W).width, collision = false, delay = myHero:GetSpellData(_W).delay, speed = myHero:GetSpellData(_W).speed }
	self.E = { range = 625, width = myHero:GetSpellData(_E).width, delay = myHero:GetSpellData(_E).delay, speed = myHero:GetSpellData(_E).speed }
	self.R = { range = 25000, width = myHero:GetSpellData(_R).width, delay = myHero:GetSpellData(_R).delay, speed = math.huge }
end
  
function Ronin:myMenu()
	self.Ronin = MenuElement({id = "Ronin", name = "The Joker", type = MENU, leftIcon = "https://ddragon.leagueoflegends.com/cdn/8.8.2/img/champion/Shaco.png"})
	self.Ronin:MenuElement({id = "Combo", name = "Combo", type = MENU})
	self.Ronin:MenuElement({id = "Clears", name = "Clear", type = MENU})

	-- Combo
	self.Ronin.Combo:MenuElement({id = "quse", name = "Q Settings", type = MENU, leftIcon = "https://ddragon.leagueoflegends.com/cdn/8.8.2/img/spell/Deceive.png"})
	self.Ronin.Combo.quse:MenuElement({id = "q0use", name = "Use Q Normal", value = false})
	self.Ronin.Combo.quse:MenuElement({id = "q1use", name = "Use Q Extend", value = false})
	self.Ronin.Combo.quse:MenuElement({id = "spells", name = "Use Q for Spells to block", type = MENU})
	self.Ronin.Combo.quse.spells:MenuElement({id = "qblock", name = "Use Q Spell block", value = true})
	self.Ronin.Combo:MenuElement({id = "wuse", name = "Use W", value = true, leftIcon = "https://ddragon.leagueoflegends.com/cdn/8.8.2/img/spell/JackInTheBox.png"})
	self.Ronin.Combo:MenuElement({id = "euse", name = "Use E", value = true, leftIcon = "https://ddragon.leagueoflegends.com/cdn/8.8.2/img/spell/TwoShivPoison.png"})
	self.Ronin.Combo:MenuElement({id = "rset", name = "R Settings", type = MENU, leftIcon = "https://ddragon.leagueoflegends.com/cdn/8.8.2/img/spell/HallucinateFull.png"})
	...
	-- Clear
	self.Ronin.Clears:MenuElement({id = "euse", name = "Use E", value = true, leftIcon = "https://ddragon.leagueoflegends.com/cdn/8.8.2/img/spell/TwoShivPoison.png"})
	self.Ronin.Clears:MenuElement({id = "wuse", name = "Use W", value = true, leftIcon = "https://ddragon.leagueoflegends.com/cdn/8.8.2/img/spell/JackInTheBox.png"})
...
function Ronin:OnTick()
  ...
  if self:IsReadyToCast(_Q) and self.Ronin.Combo.quse.spells.qblock:Value() and self.SpellsLoaded == true then
...
function Ronin:Combo(target)
	if self.target == nil then return end
	  --Q
	  if self:IsReadyToCast(_Q) and not self:IsValid(self.target, myHero.pos, self.Q.range) and self:IsValid(self.target, myHero.pos, 1000) and self.Ronin.Combo.quse.q1use:Value() then
		Control.CastSpell(HK_Q, self.target)
	  elseif self:IsReadyToCast(_Q) and self:IsValid(self.target, myHero.pos, self.Q.range) and self.Ronin.Combo.quse.q0use:Value() then
		Control.CastSpell(HK_Q, self.target)
	  end
	  --W
	  ...
	  if self:IsReadyToCast(_W) and self:IsValid(self.target, myHero.pos, 450) and self.Ronin.Combo.wuse:Value() then
...
function Ronin:Clear()
  ...
  if qMinion ~= nil and self:IsReadyToCast(_W) and self.Ronin.Clears.wuse:Value() and
    self:GetManaPercent(myHero) > self.Ronin.Clears.usemana:Value() then
    Control.CastSpell(HK_W, qMinion)
  end
end
