...
function RoninDiana:myMenu()
  ...
  self.Ronin = MenuElement({id = "Ronin", name = "Ronin Diana", type = MENU, leftIcon = })
  ...
  -- Keys
	...
	-- Combo
	self.Ronin.Combo:MenuElement({id = "quse", name = "Use Q", value = true, leftIcon = self..Q})
	self.Ronin.Combo:MenuElement({id = "wuse", name = "Use W", value = true, leftIcon = self..W})
	self.Ronin.Combo:MenuElement({id = "euse", name = "Use E", value = true, leftIcon = self..E})
	self.Ronin.Combo:MenuElement({id = "ruse", name = "Use R", value = false, leftIcon = self..R})
  self.Ronin.Combo:MenuElement({id = "rsmart", name = "Use Smart R", value = true, leftIcon = self..R})

	-- Clear
	self.Ronin.Clears:MenuElement({id = "quse", name = "Use Q", value = true, leftIcon = self..Q})
  self.Ronin.Clears:MenuElement({id = "wuse", name = "Use W", value = true, leftIcon = self..Q})
	self.Ronin.Clears:MenuElement({id = "usemana", name = "Mana Usage", value = 25, min = 0, max =      100, step = 1, leftIcon = self..Tear})
  self.Ronin.Clears:MenuElement({id = "sliderminion", name = "Minions", value = 2, min = 0, max =      10, step = 1, leftIcon = self..Tear})
end
...
function RoninDiana:Combo(target)
  ...
  if self:IsReadyToCast(_Q) and self:IsValid(self.Q.range, self.target) and self.Ronin.Combo.quse:Value() then
    Control.KeyUp(HK_Q, self.target)
  end
  -- R -- Exist this on Ext? --and GotBuff(target,"dianamoonlight") >= 1--
  if self:IsReadyToCast(_R) and self:IsValid(self.R.range, self.target) and self.Ronin.Combo.rsmart:Value() and GotBuff(target, "dianamoonlight") >= 1 then
...
function RoninDiana:Clear()
	...
	if qMinion ~= nil and self:IsReadyToCast(_Q) and self.Ronin.Clears.useQ:Value() and
